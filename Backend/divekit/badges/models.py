...
class Badge(models.Model):
    ...
    module = models.ForeignKey(
        Module, on_delete=models.SET_NULL, null=True, default=None)
...
class BadgeAdmin(admin.ModelAdmin):
    list_display = ['link_to_pk', "link_to_name",
                    'description', "link_to_module"]
...
def update_user_progress(sender, **kwargs):
    ...
    user_badges = UserBadge.objects.filter(
        badge__pk=updated_badge.pk, progress__gt=updated_badge.milestones).all()
    if user_badges:
    ...
    def clean(self):
        ...
        if self.progress > self.badge.milestones:
            raise ValidationError(
                "Progress cannot be bigger than milestone count!")
    ...
    def validate_unique(self, exclude=None):
        if self.badge.is_unique:
            if UserBadge.objects.exclude(pk=self.pk).filter(owner=self.owner, badge=self.badge):
                raise ValidationError(
                    'This Badge (%s-%s) is unique and therefore cannot be assigned twice to a user' % (self.badge.id, self.badge.name))
        return super(UserBadge, self).validate_unique(exclude=exclude)


class UserBadgeAdmin(admin.ModelAdmin):
    list_display = ["link_to_pk", "link_to_user", 'link_to_badge',
                    "get_progress", "get_last_progress_at", "earned", "earned_at"]

    search_fields = ['pk', "badge__name__icontains", "last_progress_at__icontains",
                     "progress", "badge__milestones", "owner__username__icontains"]
    readonly_fields= ["earned_at","earned","last_progress_at"]
